...
def load_initial_data(sender, **kwargs):
    if not Level.objects.exists():
        call_command('loaddata', 'apps/tests_engine/fixtures/initial_level_data.json')
    if not Test.objects.exists():
        call_command('loaddata', 'apps/tests_engine/fixtures/initial_test_data.json')
    if not Question.objects.exists():
        call_command('loaddata', 'apps/tests_engine/fixtures/initial_quiz_data.json')
    if not PlacementTest.objects.exists():
        call_command('loaddata', 'apps/tests_engine/fixtures/initial_placementtest_data.json')
    if not PlacementTest_Question.objects.exists():
        call_command('loaddata', 'apps/tests_engine/fixtures/initial_placementtest_question_data.json')
